...
from xmmspider.items import TmallShopInfoItem


class TmallShopSpider(Spider):
    name = "TmallShopSpider"
    shop_urls = []

    def __init__(self, ri=None, *args, **kwargs):
        super(TmallShopSpider, self).__init__(*args, **kwargs)
        self.run_id = ri

    def start_requests(self):
        if self.shop_urls != None:
            for url in self.shop_urls:
                item = TmallShopInfoItem()
