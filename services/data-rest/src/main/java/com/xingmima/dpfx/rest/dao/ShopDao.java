...
public interface ShopDao {
     ...
     *@return
     */
    @Select("SELECT tf.id, tf.uid, tf.shopid, ts.logo_url as logoUrl, ds.title FROM t_follow tf LEFT JOIN t_shop ts ON ts.shopid = tf.shopid LEFT JOIN d_shop ds ON ds.shopid = ts.shopid WHERE ds.date = #{date} AND is_binding = 1 AND `status` = 0 AND uid = #{uid}")
    @Options(useCache = true, timeout = 10000, flushCache = false)
     ...
     *@return
     */
    @Update("update t_follow set status = 1 where uid = #{uid} and shopid = #{shopid} and status = 0 and is_binding = 1")
     ...
     *@return
     */
    @Insert("INSERT INTO `t_shop` VALUES (#{id}, #{shopid}, #{sellerId}, #{securityId}, #{nick}, #{cid}, #{cCid}, #{storeUrl}, #{logoUrl}, #{category}, #{type}, #{location}, #{serviceNumber}, #{storeDate}, #{status}, #{lastTimes}, now(), now())")
