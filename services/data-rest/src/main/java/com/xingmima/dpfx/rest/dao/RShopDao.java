...
public interface RShopDao {
     ...
     * @return the shop pv
     */
    @Select("SELECT 'day1' AS k, SUM(total_pv) FROM `r_shop` WHERE `date` >= #{date1} AND `date` < #{date2} AND shopid = #{shopid} UNION SELECT 'day2' AS k, SUM(total_pv) FROM `r_shop` WHERE `date` >= #{date3} AND `date` < #{date4} AND shopid = #{shopid}")
    HashMap getShopPv(@Param("shopid") Long shopid, @Param("date1") Integer date1, @Param("date2") Integer date2, @Param("date3") Integer date3, @Param("date4") Integer date4);
